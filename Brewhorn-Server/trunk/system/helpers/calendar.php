...
class CI_Calendar {
	 ...
	 * @return	string
	 */
	function generate($year = '', $month = '', $data = array(),$profile1='')
	{
		...
		while ($day <= $total_days)
		{
			...
			for ($i = 0; $i < 7; $i++)
			{
				...
				{ 
				$eve_dt=$month.'/'.$day.'/'.$year;
				$this->temp['cal_cell_start']='<td id="col_'.$day.'"';
				if($profile1=='self')
				$this->temp['cal_cell_start'] .='ondblclick="open_add(event,\'col_'.$day.'\',\''.$eve_dt.'\');"';
				$this->temp['cal_cell_start'] .='>';
				//$this->temp['cal_cell_start']='<td align="left" valign="top" class="calendar-day" id="col_'.$day.'" >';
				$this->temp['cal_cell_start_today']='<td id="col_'.$day.'" ondblclick="open_add(event,\'col_'.$day.'\',\''.$eve_dt.'\');">';
